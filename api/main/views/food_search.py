...
def food_search(request: Request) -> Response:
    if not (search_term := request.query_params.get("search_term")):
        return data_response(data={"matches": []})

    food = Food.objects.filter(name__icontains=search_term, user=request.user).order_by(
        Length("name").asc()
    )[:MAX_AUTOCOMPLETE_MATCHES]

    return data_response(data={"matches": [f.name for f in food]})
